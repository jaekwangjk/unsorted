		  ...
		  int nrhs, const mxArray *prhs[])
{
  /*
   *
   *You need to check how the neighbors are defined !
   *
    MATLAB INTERFACE:
  ...
  */
  ...
  /* Loop over grains: */
  for (k=0;k<N;k++){
    gind = mxGetCell(prhs[0],k);//JK note-- prhs[0] is the first argument, which is grains 
    pgind = (double *) mxGetData(gind); //p in pgind stands for the "pointer"
    
    gcval = mxGetCell(prhs[0],2*N+k);
    pgcval = (double *) mxGetData(gcval);
        //mxGetM(C) number of rows in mxArray
    ...
    for (i=0;i<m;i++) { /* Loop over pixels in buffered grain. */
      ...
      // to follow c convenction, it takes minus 1. 
      // note that this is not label, but index that is one on one to grid. 
  ...
  ndim = 2;
  vecdim[0] = dims; //size of presence is the size of grid
  vecdim[1] = 3;
